...
4. **早見表/共有資料も正本から直接生成・照合する**（二次資料・スナップショット・監査サマリから作らない）。
   2026-07に edge_cheatsheet.html を二次資料から作った結果、⑫欠落・質序列の逆転（①B最上位を最下位に）・
   ②の売ってはいけないREIT除外の欠落・⑩R運用ゲート欠落 が発生した実例あり。
5. **照合は単一パス（該当エッジ節だけ）でなく正本"全体横断"で**。別セクション（不採用台帳・分足注・監査注記・鉄則）に
   上書き/除外条件（例: ①Bの激深≥10%GD除外 L171/L946）があり、節だけ見ると取りこぼす。
6. **ユーザーが「共有したい」と言ったら＝最新版の正本 docs/edge_playbook.md を渡す**（共有資料の第一候補は正本本体）。
7. **学びは都度・確認なしで正本へ記録**（ユーザーへの逐一確認は不要。2026-07ユーザー恒久指示）。
...
**マージ権限（2026-06 ユーザー恒久許可）**: 基本的にマージはユーザーの許可を都度取らず、Claudeの判断で実行してよい。
