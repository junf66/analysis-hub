...
def summarize(
    ...
    *,
    ...
    vals = metric_values(records, metric)
    st = _stats(vals)
    print("=" * 60)
    ...
    if plot_cumul:
        ...
        print()
        for line in _ascii_cumul(metric_values(ordered, metric)):
            print(line)
...
def group_table(
    ...
    *,
    ...
    print("=" * 74)
    print(f"group_by={group_by}  metric={metric}  total filtered={len(records)}")
    print("-" * 74)
    ...
    for k, recs in groups.items():
        rows.append((k, _stats(metric_values(recs, metric)), len(recs)))
    # |t| 降順 (n>=3 のみ、それ以外は末尾)
